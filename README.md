...
| `Installer.bas` | 标准模块 | **新增** — 一键导入安装器 |
| `Tests.bas` | 标准模块 | **新增** — 完整单元测试用例 |
...
4. 先开启权限：**工具 → 引用 → 勾选** `Microsoft Visual Basic for Applications Extensibility 5.3`
5. 再开启信任：**文件 → 选项 → 信任中心 → 信任中心设置 → 宏设置 → 勾选** `信任对 VBA 项目对象模型的访问`
6. 插入一个新模块，粘贴 `Installer.bas` 中的代码
7. 运行 `Installer.InstallFromFolder`，选择解压后的代码文件夹
8. 所有模块和类会自动导入，最后**另存为 `.xlsm`** 即可

> 如果代码放在桌面，直接运行 `Installer.QuickInstall` 更方便。

### 方式二：手动导入

在 VBA 编辑器中，**文件 → 导入文件**，逐个导入所有 `.bas` 和 `.cls` 文件，然后另存为 `.xlsm`。
